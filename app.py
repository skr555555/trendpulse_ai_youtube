...
from src.analysis.sentiment import get_sentiment
from src.analysis.trends import get_top_videos, get_trends_over_time
from src.analysis.influencers import get_top_channels
...
def detect_language(text):
    ...
    except (LangDetectException, TypeError): return 'unknown'
...
def load_and_process_data(keyword, video_limit, comment_limit):
    ...
    if df.empty:
        st.info("Fetching new data from YouTube API...")
        collector = YouTubeCollector()
        df = collector.fetch_video_data_by_keyword(keyword, video_limit, comment_limit)
        if not df.empty: db_handler.insert_data(df)
    
    if df.empty: return None

    df['language'] = df['comment_text'].apply(detect_language)
    df['cleaned_text'] = df.apply(lambda row: clean_text_multilingual(row['comment_text'], row['language']), axis=1)
    df['sentiment'] = df.apply(lambda row: get_sentiment(row['cleaned_text']) if row['language'] == 'en' else 'N/A (Non-English)', axis=1)
    
    return df

st.title("📺 TrendPulse AI: Multilingual YouTube Analytics")
with st.sidebar:
    st.header("⚙️ Controls")
    keyword_input = st.text_input("Enter Search Keyword", value="Nvidia")
    video_limit = st.slider("Videos to Scan", 5, 50, 10)
    comment_limit = st.slider("Comments per Video", 10, 100, 20)
    analyze_button = st.button("Analyze Trends", type="primary")
...
if analyze_button and keyword_input:
    ...
    if df is not None and not df.empty:
        ...
        # tab1, tab2, tab3 = st.tabs(["📊 Overview", "🏆 Top Content & Channels", "📝 Data"])
        tab1, tab2, tab3, tab4 = st.tabs(["📊 Overview", "🏆 Top Content", "📺 Top Channels", "📝 Raw Data"])
        
        with tab1:
            col1, col2 = st.columns(2)
            with col1:
                st.subheader("Sentiment (English Comments Only)")
                sentiment_counts = df[df['sentiment'] != 'N/A (Non-English)']['sentiment'].value_counts()
                if not sentiment_counts.empty:
                    fig = px.pie(sentiment_counts, values=sentiment_counts.values, names=sentiment_counts.index, title="Sentiment Breakdown")
                    st.plotly_chart(fig, use_container_width=True)
                else:
                    st.info("No English comments found to analyze sentiment.")
            with col2:
                st.subheader("Most Frequent Words (All Languages)")
                text = " ".join(df['cleaned_text'])
                if text:
                    wordcloud = WordCloud(width=800, height=400, background_color='white').generate(text)
                    fig, ax = plt.subplots(); ax.imshow(wordcloud, interpolation='bilinear'); ax.axis('off')
        ...
        with tab3:
            ...
            st.dataframe(influencers_df, use_container_width=True)
        with tab4:
            st.subheader("Processed Comment Data")
            st.dataframe(df[['comment_text', 'language', 'cleaned_text', 'sentiment', 'channel_title']])
    else:
        st.error(f"No data found for '{keyword_input}'.")